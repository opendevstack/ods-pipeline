...
```
NOTE: If you are required to have certificates installed on your machine, e.g. due to being in an enforcing
corporate network, place the `*.crt` files into the `build/package/certs` folder. It is being picked up from there when
needed. The content of this folder is ignored by `git`.
