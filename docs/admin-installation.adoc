...
* `sops` and `age` for the helm secrets plugin (link:https://github.com/mozilla/sops/releases[sops] and link:https://github.com/FiloSottile/age#installation[age]).
...
cat images-chart/secrets.yaml | SOPS_AGE_KEY_FILE=./key.txt helm secrets edit secrets.images.yaml
