...
* **deploy**: OpenShift/K8S resource definitions, such as `Task` resources. The tasks typically make use of the images built via `build/package` and released to ghcr.io. Their `script` calls one or more executables built from the `cmd` folder.
