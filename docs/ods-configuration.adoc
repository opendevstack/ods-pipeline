...
Once both `ods.yml` and webhook configuration exist, any push in that repo will trigger the pipeline described in `ods.yml`.

== Reference

The configuration in `ods.yml` allows four top-level fields:
