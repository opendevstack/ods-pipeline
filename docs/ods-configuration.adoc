...
The following example overrides compute resources of a remotely resolved task and adds an additional environment variable (provided `enable-api-fields` is set to `alpha`):
