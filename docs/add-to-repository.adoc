...
The only Go specific task is `ods-build-go-v0-4-0`, if you use another technology, exchange the task for another one such as `ods-build-gradle-v0-4-0`.

The `ods-package-image-v0-4-0` task assumes the Docker context in the `docker` directory, and the `Dockerfile` located inside that folder. Therefore, create a directory `docker` and create a `Dockerfile` inside like this one:
...
NOTE: `ods-build-go-v0-4-0` produces a binary called `app` and places it at `docker/app` so that it can be referenced in the `Dockerfile` and copied into the image by the `ods-package-image-v0-4-0` task.

== Create Helm Chart

The `ods-deploy-helm-v0-4-0` requires a Helm chart to be present to deploy the Kubernetes resources for the application. It is recommended to start with the link:https://github.com/opendevstack/ods-pipeline/tree/sample-helm-chart[sample chart provided in this repository].
