...
Runs `npm run build`, and copies the folder specified in `build-dir` into directory `dist` which is placed into the directory specified by `output-dir`.

If `copy-node-modules` is `true` the `node_modules` folder is also copied into the mentioned dist directory.
