...
* `cache-dependencies`: enables having the go module cache on the PVC's cache area. See https://go.dev/ref/mod#module-cache.
...
a| If `cache-dependencies` is `'true'` sets environment variable `GOMODCACHE` to `.ods-cache/deps/gomod` so that the go module cache is placed at the appropriate caching area on the PVC (see https://go.dev/ref/mod#module-cache)
...
The task cleans the cache which deletes any files in unknown cache areas. A build task may store cached dependencies under directory `./ods-cache/deps/<technology-name>/` where technology-name provides a namespace. For example this could be 'npm' if at some point in the future this would be supported.
...
| Container image to build TypeScript applications. Based on `ubi8/nodejs-16` (SDS-EXT-26), includes SDS-EXT-16, SDS-SHARED-3, SDS-TASK-18 and SDS-TASK-28.
