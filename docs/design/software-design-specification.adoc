...
Processes tags specified in the `extra-tags` parameter and adds missing to the image in the target registry:

* Uses skopeo (SDS-EXT-17) to inspect image in target registry to determine whether there are tags defined in `extra-tags` which have not yet been applied.
* For each missing tag:
    * Uses skopeo to add missing tag to the image in the target registry.
    * The resulting image name and tag is placed into `.ods/artifacts`.
|===
...
a| The script is supposed to be downloaded and piped into bash. The script installs the prerequisites not already present in the Web Terminal image (`helm-diff`, `helm-secrets`, `sops`, `age`), clones the Git repository and then runs `./install.sh`.
|===
