...
| SRS-TASK-START-6
| The task shall support caching. Caching shall enable persisting build dependencies. Caching shall enable to reuse build outputs and reports if a build tasks runs when the associated working directory has not changed.
|===
