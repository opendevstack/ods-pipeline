...
| If an Aqua scanner download URL is provided, the pushed image shall be scanned with the Aqua scanner binary.
...
| The task shall skip when no namespace parameter is given.
...
* Value files corresponding to the target namespace shall be applied.
* Encrypted secret files shall be decrypted on the fly and applied.
* The "app version" shall be set to the checked out Git commit SHA.
* Charts in any subrepositories shall be added as a subchart.
