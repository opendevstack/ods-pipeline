...
The example project is deployed on a single OpenShift cluster which has three OpenShift projects to deploy into: `todo-dev`, `todo-qa` and `todo-prod`. ODS pipeline is installed in the `todo-cd` project as per the link:user-installation.adoc[User Installation Guide].
